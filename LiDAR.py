...
class LiDAR:
    def __init__(self, angle_interval=[0, 2*math.pi], position = (0,0), reach=10):
    ...
    def measure_at_angle(self, angle, obstacles): #function for measuring the distance between the robot position in a certain direction

        max_reach_point = (self.position[0] + (self.reach*np.cos(angle)), self.position[1] + (self.reach*np.sin(angle))) #finding the maximum point for computing intersections
        ray = ((self.position[0], self.position[1]),(max_reach_point[0], max_reach_point[1])) #generating the ray

        measured_distance = 2*self.reach #dummy value for beginning the procedure
        intersection_point = None

        for obstacle in obstacles: #repeat for each obstacle
          ...
          if not R: #if there is no intersection, the "out of range" value is sensed
            ...
            distance = math.dist(self.position, R) #computing the distance between the actual position and the intersection
            if distance < measured_distance and distance <= self.reach: #if it is the current minimum distance and it is in reach
              if ((self.position[0]<=R[0]<=max_reach_point[0]) or (max_reach_point[0]<=R[0]<=self.position[0])) and ((self.position[1]<=R[1]<=max_reach_point[1]) or (max_reach_point[1]<=R[1]<=self.position[1])):
    ...
    def measure(self, n, obstacles):
        if n<=0:
            raise ValueError('The number of rays must be positive')
        
        angles = np.linspace(self.angle_interval[0], self.angle_interval[1], n+1)
        measurements = {}

        for angle in angles:
          measurement, intersection_point = self.measure_at_angle(angle, obstacles)
